...
import { FALLOUT_ZERO } from './helpers/config.mjs';
...
Hooks.once('init', function () {
  ...
  CONFIG.FALLOUT_ZERO = FALLOUT_ZERO;
  ...
  Actors.registerSheet('fallout-zero', FalloutZeroActorSheet, {
    makeDefault: true,
    label: 'FALLOUT_ZERO.SheetLabels.Actor',
  });
  Items.unregisterSheet('core', ItemSheet);
  Items.registerSheet('fallout-zero', FalloutZeroItemSheet, {
    makeDefault: true,
    label: 'FALLOUT_ZERO.SheetLabels.Item',
  });
...
async function createItemMacro(data, slot) {
  ...
  if (!macro) {
    macro = await Macro.create({
      ...
      img: item.img,
      command: command,
      flags: { 'fallout-zero.itemMacro': true },
    });
  }
